...
{
    fprintf(stderr,
        "Usage: %s <func-id> <x-begin> <x-end> <num-points> <num-clients> <port> <server-timeout-ms>\n"
...
int main(int argc, char** argv)
{
    ...
    parse_int_arg(argv[7], "server-timeout-ms", 1);

    char points_file[128];
    snprintf(points_file, sizeof(points_file), "build/library_points_%s.txt", port);
